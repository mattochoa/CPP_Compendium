...
**Domain.** One of 17 regions (D00–D16) plus Sources (SRC) and Practice (PRX). Each domain answers one question (see `domains.yaml` and Home).
...
- **Maps** use the prefix `Map — `; **source guides** use `Guide — `; **paths** use `Path — `.
- **Folders:** `01 Maps/` · `02 Notes/NN Domain Name/` · `03 Sources/` · `04 Practice/` · `00 System/` (protocols, templates, dashboards, ledger, briefs). `__RESOURCES__/` holds the books (git-ignored).
