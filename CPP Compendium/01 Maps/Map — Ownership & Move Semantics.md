...
> A deep copy is always safe and always correct, but for a large buffer it is O(*n*) work to throw away a moment later. `shared_ptr`'s reference count is always safe under single-threaded sharing, but it costs an atomic operation on every copy whether or not you needed shared ownership. This domain's tools — move, `unique_ptr`, `shared_ptr` — are C++'s answer to *safe by default, cheap when the default is unnecessary*: the [[Map — What C++ Is|zero-overhead principle]] applied to ownership.
